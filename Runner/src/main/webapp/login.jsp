  ...
  <div id="header">
    ...
    <div id="mainnav">
      ...
      <span>
							<form action="login.action" method="post">
								<table width="100%">
									<tr>
										...
										<td>
											<img src="captchaServlet" height:20px; width:60px"/>
										</td>
										<td>
											<a href="aaa.action">看不清？</a>
										</td>
										<td>
			               ...
			               <label>
                                    <br/>
                                    <c:if test="${error eq 'fail1'}">
                                        <span style="color: red" id="back_data">用户名或者密码为空</span>
                                    </c:if>
                                    <c:if test="${error eq 'fail2'}">
                                        <span style="color: red" id="back_data">用户名或者密码错误</span>
                                    </c:if>
                                    <c:if test="${error eq 'fail3'}">
                                        <span style="color: red" id="back_data">验证码错误</span>
                                    </c:if>                       
...
到今天为止，盖茨和他的妻子Melinda Gates建立的基金已经将25多亿美元用于了全球的健康事业，将14亿多美元用于改善人们的学习条件，其中包括为盖茨图书馆购置计算机设备、为美国和加拿大的低收入社区的公共图书馆提供互联网培训和互联网访问服务             </p>
	 ...
	 <table width="300">
      ...
      <c:forEach items = "${point}" var = "item"  varStatus="status">
        <li><a href="#">${item.getNickname()}&nbsp&nbsp&nbsp&nbsp${item.getPoint()}</a></li>
	  </c:forEach> 
      </ul>
     </div>
     <div class="clear"></div>
