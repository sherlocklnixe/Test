  ...
  <div id="header">
    ...
    <div id="mainnav">
      ...
      <span><table width="100%" border="0">
								<tr>
									<td align="right">
										上一次登录时间：中国标准时间 ${memberinfo.getLatestdate()}
									</td>
									<td align="center">
										欢迎，&nbsp;
										<a href="isCreateSpace.action"><font color="red">
												${memberinfo.getNickname()}</font> </a>&nbsp;
									</td>
									<td align="left">
										当前您的积米：&nbsp;&nbsp;
										<font color="red">${memberinfo.getPoint()} </font>&nbsp;&nbsp;
										当前等级：
										<font color="red">
											<c:if test="${memberinfo.getGradeid() eq '1'}">
                                        		业余爱好者
		                                    </c:if>
		                                    <c:if test="${memberinfo.getGradeid() eq '2'}">
		                                      	  跑步小将  
		                                    </c:if>
		                                    <c:if test="${memberinfo.getGradeid() eq '3'}">
		                                                                                   跑步健将
		                                    </c:if>
		                                     <c:if test="${memberinfo.getGradeid() eq '4'}">
		                                                                                 专业运动员
		                                    </c:if>
		                                     <c:if test="${memberinfo.getGradeid() eq '5'}">
		                                                                                 顶级运动员
		                                    </c:if>
                                   		 </font>
									</td>
        ...
        <div id="hots">
          ...
          <li>
           <div>
            ...
            <a href="modify2.action?nickname=${nickname}">基本信息</a>
            <p>可修改自己的基本信息</p>
           </div>
          </li>
          <li>
           <div>
            ...
            <a href="inbox1.action?nickname=${nickname}">我的信箱</a>
            <p>写信息、收件箱、发件箱</p>
           </div>
          </li>
          <li>
           <div>
            ...
            <a href="buddyList1.action?nickname=${memberinfo.getNickname()}">我的好友</a>
            <p>好友管理及黑名单</p>
           </div>
          </li>
          <li>
           <div>
            ...
            <a href="judgeSpace1.action?nickname=${nickname}">个性空间</a>
            <p>创建自己的个性空间</p>
           </div>
          </li>
          </ul>
	 ...
	 <table width="300">
      ...
      <ul>
       <c:forEach items = "${point}" var = "item"  varStatus="status">
        <li><a href="judgeSpace2.action?friendname=${item.getNickname()}">${item.getNickname()}&nbsp&nbsp&nbsp&nbsp${item.getPoint()}</a></li>
	  </c:forEach> 
      </ul>
     </div>
     <div class="clear"></div>
